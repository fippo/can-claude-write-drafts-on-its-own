...
docname: draft-claude-read-the-gerrit-cl-latest
...
venue:
  ...
  github: fippo/mimi
  latest: https://fippo.github.io/mimi/draft-claude-read-the-gerrit-cl.html
...
streams over RTP. The syntax described here is the one that has been
deployed in WebRTC implementations since 2019; its parameters reuse the
terminology of the Ogg Opus channel mapping in RFC 7845.
...
Section 5.1.1 of {{!RFC7845}}, and the parameters defined in this
document deliberately reuse that terminology.
...
To fill this gap, WebRTC implementations introduced, in 2019, an
"audio/multiopus" media subtype carried over RTP as a new code path that
does not interfere with the standard mono/stereo Opus decoder. It uses
the RTP channel count to carry the number of decoded channels and three
additional format parameters --- "num_streams", "coupled_streams", and
"channel_mapping" --- to carry the multistream configuration. For
example, a 5.1 surround configuration (six channels) is described as:

~~~
multiopus/48000/6 channel_mapping=0,4,1,2,3,5 num_streams=4 \
coupled_streams=2
~~~

This document formalizes that deployed usage and registers the
corresponding media subtype and parameters with IANA, updating
{{RFC7587}}.
...
* The number of channels encoded in the "a=rtpmap" line carries the
  ...
  a=fmtp:100 channel_mapping=0,4,1,2,3,5; num_streams=4; \
             coupled_streams=2
  ~~~
...
with the "multiopus" encoding name will simply not match such an
implementation's Opus codec, and the corresponding format will be
ignored during negotiation. This is by design: the multistream code
path is separate from the standard Opus decoder. A separate
"audio/opus" format may be offered alongside an "audio/multiopus" format
to interoperate with mono/stereo endpoints.
...
layout that the sender of the parameters uses and that the corresponding
decoder expects. They are not subject to negotiation in the sense of
intersection; an answerer that cannot decode the offered configuration
omits or rejects the format rather than modifying these parameters. This
is analogous to the handling of the declarative "ptime" attribute and
the directional Opus parameters in {{RFC7587}}.
...
The semantics are those of Section 5.1.1 of {{RFC7845}} and the Opus
multistream API:
...
The "audio/multiopus" SDP syntax formalized here was introduced in the
WebRTC project in 2019 (tracked as bug webrtc:8649), reusing the
terminology of the Ogg Opus channel mapping {{RFC7845}}. The multistream
Opus mechanism itself originates with the Opus codec {{RFC6716}}.
